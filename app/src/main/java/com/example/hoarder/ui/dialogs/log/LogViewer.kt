...
class LogViewer(private val activity: AppCompatActivity, private val logRepository: LogRepository) {
    ...
    fun showLogDialog(logType: String) {
        ...
        val logAdapter = LogAdapter(activity, emptyList(), logEntryFormatter) { copyText ->
    ...
    private fun updateNoDataView(parent: ViewGroup, show: Boolean) {
        if (noDataView == null) {
            noDataView = TextView(activity).apply {
                text = "No logs found."
                textSize = 14f
                setTextColor(ContextCompat.getColor(activity, R.color.amoled_light_gray))
