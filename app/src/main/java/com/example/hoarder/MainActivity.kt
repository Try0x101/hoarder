...
class MainActivity:AppCompatActivity(){
    ...
    private val dr=object:BroadcastReceiver(){
        override fun onReceive(c:Context?,i:Intent?){
            i?.getStringExtra("jsonString")?.let{j->ld=j;if(ui.isDataVisible())dj(j)}
        }
    }

    private val ur=object:BroadcastReceiver(){
        override fun onReceive(c:Context?,i:Intent?){
            ui.updateStatus(i?.getStringExtra("status"),i?.getStringExtra("message"),
    ...
    override fun onResume(){
        super.onResume()
        NotifUtils.createSilentChannel(this)
        if(permHandler.hasAllPerms()&&!DataUtils.isServiceRunning(this,BackgroundService::class.java)){ss();rs()}
    }
    ...
    fun dj(j:String){
        try{dt.text=g.toJson(JsonParser.parseString(j))}
        catch(e:Exception){dt.text="Error formatting JSON: ${e.message}"}
    }
